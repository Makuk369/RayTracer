...
void Game::Run(){
	...
	while(isRunning){
		...
		if(updateDelay <= 0.f){
			...
			SetPixel(mWindowSurface, mouseX, mouseY, 0, 0, 0);
...
void Game::SetPixel(SDL_Surface* surface, int x, int y, uint8_t r, uint8_t g, uint8_t b){
	SDL_LockSurface(surface);
	SDL_PixelFormat pixelFormat = surface->format;
	const SDL_PixelFormatDetails *pixelFormatDetails = SDL_GetPixelFormatDetails(pixelFormat);
	uint8_t* pixelArray = (uint8_t*)surface->pixels;
	pixelArray[y*surface->pitch + x*pixelFormatDetails->bytes_per_pixel + 0] = 0; // g
	pixelArray[y*surface->pitch + x*pixelFormatDetails->bytes_per_pixel + 1] = 0; // b
	pixelArray[y*surface->pitch + x*pixelFormatDetails->bytes_per_pixel + 2] = 255; // r
