...
{
    glm::vec3 scatterDir = hitRec.normal + glm::normalize(RTUtils::RandomVec3());
...
*/
bool Lambertian::Scatter(const Ray& ray, const HitRecord& hitRec, glm::vec4& color, Ray& scatteredRay) const
{
    glm::vec3 scatterDir = hitRec.normal + glm::normalize(RTUtils::RandomVec3());
...
Metal::Metal(const glm::vec4& albedo, float roughness)
...
bool Metal::Scatter(Ray& ray, const HitRecord& hitRec, glm::vec3& color) const
{
    ...
    reflectDir = glm::normalize(reflectDir) + (mRoughness * glm::normalize(RTUtils::RandomVec3()));

    ray.origin = hitRec.position + hitRec.normal * 0.0001f;
    ray.direction = reflectDir;
    color = mAlbedo;

    return (glm::dot(ray.direction, hitRec.normal) > 0);
}
*/
bool Metal::Scatter(const Ray& ray, const HitRecord& hitRec, glm::vec4& color, Ray& scatteredRay) const
{
    glm::vec3 reflectDir = glm::reflect(ray.direction, hitRec.normal);
